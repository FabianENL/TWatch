...
public:
    ReleaseEvent(int16_t x, int16_t y, int16_t startX, int16_t startY)
        : m_x(x), m_y(y), m_startX(startX), m_startY(startY) {}
...
public:
    ...
    inline SwipeDirection getSwipeDirection() const {
        ...
        if (abs(dx) > abs(dy)) {
            if (dx > 0) return SwipeDirection::Right;
            else if (dx < 0) return SwipeDirection::Left;
        } else {
            if (dy > 0) return SwipeDirection::Down;
            else if (dy < 0) return SwipeDirection::Up;
        }
